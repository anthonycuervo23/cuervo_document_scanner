...
class PushNotificationService {
  ...
  registerNotificationListeners() async {
    ...
    var androidSettings = const AndroidInitializationSettings('ic_notifications');
  ...
  Future<void> sendLocalNotification({RemoteMessage? message}) async {
    ...
    Future<String> _base64encodedAssetImage(String url) async {
      ByteData bytes = await rootBundle.load(url);
      var buffer = bytes.buffer;
      final String base64Data = base64Encode(Uint8List.view(buffer));
      return base64Data;
    }
    ...
    final String assetLogo = await _base64encodedAssetImage("assets/pngs/login/white_logo.jpg");
    ...
    if (notification != null && android != null) {
      ...
      if (android.imageUrl != null && (android.imageUrl?.isNotEmpty ?? false)) {
        final String bigPicture = await _base64encodedImage(imageUrl!);

        bigPictureStyleInformation = BigPictureStyleInformation(
          ByteArrayAndroidBitmap.fromBase64String(bigPicture),
          largeIcon: ByteArrayAndroidBitmap.fromBase64String(assetLogo),
          hideExpandedLargeIcon: true,
          summaryText: title.toString().boldTag(),
          contentTitle: body,
        ...
        bigTextStyleInformation = BigTextStyleInformation(
          body,
          contentTitle: title.toString().boldTag(),
          htmlFormatContentTitle: true,
          htmlFormatSummaryText: true,
      ...
      flutterLocalNotificationsPlugin.show(
        notification.hashCode,
        title.toString().removeHTMLTag(),
        body.toString().removeHTMLTag(),
      ...
      if (data?["image"] != null && (data?["image"]?.toString().trim().isNotEmpty ?? false)) {
