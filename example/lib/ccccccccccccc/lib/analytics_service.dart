...
import 'package:captain_score/common/constants/apps_flyer_constants.dart';

class AnalyticsService {
  static FirebaseAnalyticsObserver getAnalyticsObserver() => FirebaseAnalyticsObserver(analytics: firebaseAnalytics);
  ...
  Future<void> logAppEvent({String? eventName, Map<String, dynamic>? eventValues}) async {
    ...
    data.addAll(
      {
        "device_data": jsonEncode(deviceData),
        "screen_name": currentRouteName,
        "screen_args": routeArguments.toString(),
      },
    );

    // print("Loading Event : $eventName : $eventValues");

    try {
      await appsflyerSdk.logEvent((eventName ?? currentRouteName ?? '').replaceAll("/", ""), data);
      // ignore: empty_catches
    } on Exception {}

    // Google Analytics Tracking
    try {
      await firebaseAnalytics.logEvent(name: eventName ?? currentRouteName ?? '', parameters: data);
      // ignore: empty_catches
    } catch (e, stackTrace) {
  ...
  Future<void> initiatedCheckout({
    ...
    checkoutParams.addAll({
      AppsFlyerEventParamsConstants.PAYMENT_PLAN_PARAMS: paymentParams,
