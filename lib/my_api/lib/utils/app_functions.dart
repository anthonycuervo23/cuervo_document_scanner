...
import 'package:bakery_shop_admin_flutter/utils/check_storage_permission.dart';
...
class AppFunctions {
  ...
  Future<void> forceLogout({bool onClearData = false}) async {
    ...
    // if (!onClearData) {
    //   Catcher.navigatorKey?.currentState?.popUntil((route) => (route.isFirst));

    //   if ((countryCode.toString().toLowerCase() == "in") || isCRM) {
    userLoginData = null;
    await FirebaseAuth.instance.signOut();
    // await Catcher.navigatorKey?.currentState?.pushReplacementNamed(RouteList.login_screen);
    //   } else {
    //     userLoginData = null;
    //     await Catcher.navigatorKey?.currentState?.pushReplacementNamed(RouteList.gmail_login);
    //   }
    // } else {
    //   userLoginData = null;
    // }
  ...
  Future<Map<String, String>> initPlatformState() async {
    ...
    try {
      ...
      deviceData = <String, String>{
        'device_model': "test",
        'systemVersion': "notfound",
        'device_type': (!kIsWeb && Platform.isAndroid) ? "android" : "ios",
        'device_id': deviceNotificationToken.toString().isEmpty ? 'notfound' : deviceNotificationToken.toString(),
        'unique_id': "notfound",
      };
      await bakeryBox.put(HiveConstants.DEVICE_DATA, deviceData);
  ...
  Future<String> getApplicationDirectory() async =>
      kIsWeb ? '/assets' : (await path_provider.getApplicationDocumentsDirectory()).path;
  ...
  void popAllAlertDialog(BuildContext context) {
    Catcher.navigatorKey?.currentState?.popUntil((route) => isThereCurrentDialogShowing(context) == false);
  }
